...
import { NOISE_STAGES, NOISE_FAILURE_STAGES, NOISE_CATEGORIES, NOISE_DETAILS, NOISE_OUTCOMES, type NoiseDeviceFailure, type NoiseRelease, type NoiseJob, type NoiseStatus, type NoiseStartInput, type NoiseStageObservation, type NoiseStage, type NoiseStatusV2, type NoiseStartInputV2 } from "./worker-noise-diagnostic.types";
...
function parseStart<S extends NoiseStartInput["schema"] | NoiseStartInputV2["schema"]>(input: unknown, schema: S) {
  ...
  if (value.schema !== schema) throw serialFailure("noise_schema");
  return { schema, attemptId: noiseAttemptId(value.attemptId), expectedBootOrdinal: noisePositive(value.expectedBootOrdinal), networkObservedAtUs: noiseUInt(value.networkObservedAtUs), fixtureIpv4: noisePrivateIpv4(value.fixtureIpv4), fixturePort: noisePort(value.fixturePort), authorityPublicKey: noiseAuthorityKey(value.authorityPublicKey) };
}
export function parseNoiseStartInput(input: unknown): NoiseStartInput { return parseStart(input, "worker-noise-diagnostic-start-v1"); }
export function parseNoiseStartInputV2(input: unknown): NoiseStartInputV2 { return parseStart(input, "worker-noise-diagnostic-start-v2"); }
...
function release(input: unknown, maybeHorizon: number | undefined): NoiseRelease {
  ...
  if (result.startedAtUs !== null && result.deadlineAtUs !== null && result.deadlineAtUs !== (maybeHorizon ?? result.startedAtUs + 5_000_000)) throw serialFailure("noise_cleanup_deadline");
...
function job(input: unknown, v2: boolean): NoiseJob {
  ...
  const result: NoiseJob = {
    ...
    stages: stages(value.stages), firstFailure: maybeFailure(value.firstFailure), terminal: maybeTerminal(value.terminal), resources: release(value.resources, v2 ? noiseUInt(noiseUInt(value.authorityDeadlineUs) + 5_000_000) : undefined),
  };
  ...
  if (result.terminal?.outcome === "accepted") requireAccepted(result, v2);
...
function requireAccepted(value: NoiseJob, v2: boolean): void {
  ...
  for (const stage of value.stages) {
    ...
    const bounds: Partial<Record<NoiseStage, number>> = { noise_prepared: 60_000_000, tcp_connected: 5_000_000, act_one_written: 2_000_000, act_two_received: 10_000_000, proof_written: 2_000_000, ...(!v2 ? { worker_quiescent: 5_000_000 } : {}) };
...
function parseStatus<S extends NoiseStatus["schema"] | NoiseStatusV2["schema"]>(input: unknown, schema: S) {
  ...
  const state = noiseEnum(value.state, ["idle", "admitted", "running", "cancelling", "terminal"]), maybeJob = value.job === null ? null : job(value.job, schema === "worker-noise-diagnostic-status-v2");
  ...
  return { schema, state, observation, job: maybeJob };
}

/** Historical v1 reader retains cancellation-relative cleanup semantics. */
export function parseNoiseStatus(input: unknown): NoiseStatus { return parseStatus(input, "worker-noise-diagnostic-status-v1"); }
/** V2 reader uses an absolute admission+125s horizon and actual measured cleanup. */
export function parseNoiseStatusV2(input: unknown): NoiseStatusV2 { return parseStatus(input, "worker-noise-diagnostic-status-v2"); }
