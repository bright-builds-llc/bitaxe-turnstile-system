...
export type WorkerAcceptanceCampaign = { id: string; window: 0 | 1 | 2; maximumActiveMilliseconds: 180000 | 30000 };
...
function parseAcceptanceCampaign(input: unknown): WorkerAcceptanceCampaign {
  const value = exactRecord(input, ["id", "window", "maximumActiveMilliseconds"]);
  if (typeof value.id !== "string" || !/^[A-Za-z0-9_-]{21}[AQgw]$/u.test(value.id) || !Number.isSafeInteger(value.window) || ![0, 1, 2].includes(Number(value.window)) || value.maximumActiveMilliseconds !== (value.window === 0 ? 180000 : 30000)) throw new Error("Work Lease acceptance campaign is invalid");
  return { id: value.id, window: value.window as 0 | 1 | 2, maximumActiveMilliseconds: value.maximumActiveMilliseconds as 180000 | 30000 };
}
