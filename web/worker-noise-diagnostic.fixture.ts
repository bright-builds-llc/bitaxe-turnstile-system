...
import { NOISE_STAGES, type NoiseStatus, type NoiseJob, type NoiseStartInput } from "./worker-noise-diagnostic";
...
export async function noiseAdmitted(input = noiseInput): Promise<NoiseStatus> {
  const job = noiseJob();
