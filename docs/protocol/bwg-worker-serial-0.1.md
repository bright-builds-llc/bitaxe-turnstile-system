...
After possession and while no Work Lease is active, `transport_probe` accepts exactly `{padding:<ASCII x repeated zero or more times>, responsePaddingBytes:<integer>}` and returns `{padding:<ASCII x repeated to the requested response length>, requestPaddingBytes:<actual received input padding length>}` as `result`. The requested response length must be at least the input padding length and must leave room for the exact Controller response envelope. The browser must compare `requestPaddingBytes` with the sent input length; response extension cannot conceal dropped request bytes. No arbitrary input is echoed. Both complete Controller payloads must fit 65,536 bytes. The qualification helper independently fills the request and response bounds using their exact request ID, so both directions reach exactly 65,536 bytes. The probe has no mining, NVS, or configuration effect.
...
Receive failures retain only the first closed stage and a bounded observed-byte count for the logical session. A fresh admitted session clears that receipt before publication; clean close is not a receive failure. Revocation clears partial receive input before a later hello. These diagnostics neither prove which bytes reached the application nor extend authority.

Storage/HTTP observations identify the first failed SPIFFS or HTTP startup phase with a closed platform error category. A separate status record appears only after both filesystem and HTTP initialization attempts complete. Raw errors, paths, and network values never enter these records; failed outcomes cannot qualify a healthy startup.

The owner reduced the pre-mining sample from twenty to four on 2026-09-05 ([ADR-0095](../adr/0095-four-cycle-worker-qualification.md)). Already valid cycles retain their original evidence and runtime identity. A qualification-only amendment may bind separate tool/documentation revisions and protected original artifacts; it cannot change the deployed browser/firmware identities, preservation baseline, signed lease rules or 240-second cumulative device ceiling. A supervisor restart requires fresh possession on the retained page before authorization.
