...
`bwg-worker-usb/0.2` defines the local physical transport topology used by Worker Controller 0.3.
...
- Its vendor-specific `worker_control` function carries only bounded Local Device Possession and
  Controller 0.3 frames in both directions.
...
Logs, evidence, and JSON-shaped observations cannot become possession or Controller frames.
Possession proofs and Controller responses cannot be emitted through CDC evidence. A browser or
host must not probe an unadmitted CDC or bootloader function with either request type.

## Identity and reacquisition

Device Identity possession and enumeration identity are distinct facts. Moving from ROM bootloader
USB Serial/JTAG to application TinyUSB requires:

1. a fresh signature by the previously established Device Identity;
...
Proof before Controller 0.3 frames.
...
- [`fixtures.json`](../../conformance/bwg-worker-usb-0.2/fixtures.json) publishes the exact topology,
  framing facts, redacted identities, and executable reacquisition/restoration scenarios.
- [`contract.schema.json`](../../conformance/bwg-worker-usb-0.2/contract.schema.json) is the strict
  Draft 2020-12 fixture schema.
- `bwg-core/worker-controller-v03` exports the USB 0.2 topology parser with Controller 0.3.
- `bwg-core/worker-usb-v02-conformance/fixtures` and `/schema` expose package-consumable firmware/host
  artifacts.
