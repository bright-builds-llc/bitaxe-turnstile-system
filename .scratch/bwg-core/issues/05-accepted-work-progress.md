...
The in-memory tracer ledger stores stable acknowledgements by event identity and deduplicates share fingerprints. Exact event replay returns the same acknowledgement and projection; a new event identity for an existing share returns a stable `duplicate_share` acknowledgement with no credit. The same domain port is ready for the PostgreSQL transaction and gRPC adapter added by later persistence and Pool Adapter tickets.
