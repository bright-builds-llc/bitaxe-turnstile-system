...
- [ ] Define separate replaceable **Update Authority** and **Work Lease Authority** roles and keys;
  one key can never satisfy both verification interfaces.
- [ ] Publish `bwg-worker-lease-authorization/0.1` as a compact Ed25519 JWS carried inside the
  existing opaque Controller 0.3 `authorization` string, with protected type
  `bwg-worker-lease-authorization+jws`.
- [ ] Define a compact protected header bounded to `alg`, a maximum-32-character `kid`, and `typ`,
...
- [ ] Prove the maximum legal compact JWS is exactly 481 bytes and fits Controller 0.3's unchanged
  512-byte `authorization` field; publish the worked byte budget plus exact 511/512/513-byte
  boundary vectors.
- [ ] Bind Start authorization to the exact authorizationless canonical grant: operation, protocol,
...
- [ ] Publish a separate `WorkerLeaseAuthorizationContext` interface without changing
...
- [ ] Require a fresh possession-derived context before each Start following baseline restoration.
...
- [ ] Allocate each Work Lease Authority key's authorization sequence durably and monotonically.
...
- [ ] Reject wrong issuer, audience, role, key ID, operation, request digest, Lease/Challenge
...
- [ ] Add a service-local development deployment authority tool that creates two mode-`0600`
...
- [ ] Issue an Ultra 205 revision `205` Controller 0.3 capability bound to the final exact Worker USB
  0.2 application descriptor and publish its Update Authority public trust configuration for the
  Reference Client.
- [ ] Support explicit public-key overlap and retirement for both authorities. Retain each accepted
...
- [ ] Publish strict schemas, runtime parsers/verifiers, fixtures, package subpaths, byte-level
...
- [ ] Do not add a remote administration API, Device Identity registry, Controller version bump,
